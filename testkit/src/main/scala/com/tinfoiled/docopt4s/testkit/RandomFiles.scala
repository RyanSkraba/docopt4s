...
object RandomFiles {
  ...
  /** Create a randomly named text file in the specified directory with the specified contents.
    ...
    */
  def createTxtContents(rnd: Random, dir: Path, contents: String, name: Option[String] = None): Path =
    createTxtContents(
      name.map(dir / _).getOrElse(getNewFile(rnd, dir, Option("txt"))),
      contents
    )
  ...
  /** Create a named text file in the specified directory with the specified contents.
    ...
    *   the file passed in.
    */
  def createTxtContents(file: Path, contents: String): Path = {
    file.up.createDirectory(failIfExists = false)
    Using.resource(file.outputStream()) {
  ...
  def createTxtFile(
    ...
    val file = name.map(dir / _).getOrElse(getNewFile(rnd, dir, Option("txt")))
    Using.resource(file.outputStream()) { fos =>
  ...
  def createBinaryFile(
    ...
    val file = name.map(dir / _).getOrElse(getNewFile(rnd, dir, Option("bin")))
  ...
  def setTimeAttributes(path: Path, time: Long, recursive: Boolean = false): Unit = {
    ...
    val attributes = Files.getFileAttributeView(path, classOf[BasicFileAttributeView])
